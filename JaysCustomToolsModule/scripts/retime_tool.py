...
class BourneRetimingUtils(object):
    
    @classmethod
    def retime_keys(cls, retime_value, incremental, move_to_next):
        range_start_time, range_end_time = cls.get_seleted_range()
    ...
    def get_start_keyframe_time(cls, range_start_time):
        start_time = cmds.keyframe(q=True, time=(range_start_time, range_start_time))
        if start_time:
            return start_time[0]
...
class BourneRetimingUtilsUI(QtWidgets.QDialog):
    
    WINDOW_TITLE = "Jay Retiming Tool"
    ...
    def display(cls):
        if not cls.dlg_instance:
            cls.dlg_instance = BourneRetimingUtilsUI()
    ...
    def __init__(self):
        super(BourneRetimingUtilsUI, self).__init__(self.maya_main_window())
...
if __name__ == "__main__":
    ...
    # print(BourneRetimingUtils.get_last_keyframe_time())
    # BourneRetimingUtils.retime_keys(1, True, True)
