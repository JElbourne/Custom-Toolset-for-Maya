...
class JaysWatchTimeNode(om.MPxNode):
    ...
    def compute(self, plug, data):
        if plug == JaysWatchTimeNode.new_sec_hand:
            ...
            if (seconds % 1) == 0:
                rotation_in_rads = curr_sec_rot - (6.0 * 0.01745)
            else:
                rotation_in_rads = curr_sec_rot
            
            new_sec_rot = data.outputValue(JaysWatchTimeNode.new_sec_hand)
            new_sec_rot.setDouble(rotation_in_rads)
            ...
            if (seconds % 60) == 0:
                rotation = curr_min_rot - (6.0 * 0.01745)
            else:
                rotation = curr_min_rot
            
            new_min_rot = data.outputValue(JaysWatchTimeNode.new_min_hand)
            new_min_rot.setDouble(rotation)
            ...
            if (seconds % 3600) == 0:
                rotation = curr_hour_rot - (30.0 * 0.01745)
            else:
                rotation = curr_hour_rot
                        
            new_hour_rot = data.outputValue(JaysWatchTimeNode.new_hour_hand)
            new_hour_rot.setDouble(rotation)
